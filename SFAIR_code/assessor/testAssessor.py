...
def test_scheduler(t_max, properties, threshold, time_limit):
    ...
    while True:
        ...
        local_results, remote_results, ground_truth = test_dispatcher(properties)
        test_times += 1
...
def calculate_dissimilarity(result_t1, result_t2, threshold):
    ...
    #Calculate kl-divergence to test if the two distributions are different    
    kl_divergence = np.sum(signature_local * np.log(signature_local/signature_remote))
    
           
    #if divergence is more than threshold remote AI has lost its operational identity
    if kl_divergence > 0.05:
        print("Alert: Identity violation! Divergence value = ", kl_divergence)
    else:
...
# def find_corresponding_files(filename):
#    #print(filename)
#    similar_files = []
#    input_perturbed = f"input/fairness/white/"
#    for root, dirs, files in os.walk(input_perturbed):
#        #print('In the find corresponding files function', files)
#        for file in files:
#            if file.startswith(filename):
#                print(os.path.join(root, file))
#                similar_files.append(os.path.join(root, file))
#                return similar_files  # Return as soon as the first match is found
...
def select_random_input_fairness(dataset_number, input_number): 
    color = ['benign','perturbed']
    selected_files_list = []

    for item in color:
        input_folder = f"./input/robustness/{item}"

        #input_folder = f"input/robustness/"
        if item == 'benign':
            folder_items = os.listdir(input_folder)
            subfolders = [item for item in folder_items if os.path.isdir(os.path.join(input_folder, item))]
            print(subfolders)
            ...
            for subfolder in subfolders:
                ...
                files = os.listdir(subfolder_path)
                files = [file for file in files if os.path.isfile(os.path.join(subfolder_path, file))]
                #print("Test 2, Input folder path:", files)
                #print('Hey DONE') 
                # Get random files by generating mask
                random_indexes = generate_mask(int(input_number), 0, len(files) - 1)
                selected_files = [files[i] for i in random_indexes]
            ...
            print("selected_files_list", selected_files_list)
    ...
    if model_flag == 0:
        for file in files:
            with open(file, 'rb') as file_obj:
                resp = requests.post("http://localhost:5000/predict_local",
                                     files={'image': file_obj})
                results_array.append(ast.literal_eval(resp.text))
    elif model_flag == 1:
        for file in files:
            with open(file, 'rb') as file_obj:
                resp = requests.post("http://localhost:5001/predict_remote",
                                     files={'image': file_obj})
                results_array.append(ast.literal_eval(resp.text))
...
def send_test_request_efficiency(files, model_flag):
    ...
    if model_flag == 0:
        for file in files:
            ...
            with open(file, 'rb') as file_obj:
                requests.post("http://localhost:5000/predict_local",
                              files={'image': file_obj})
        ...
        for file in files:
            start_time = time.perf_counter()
            with open(file, 'rb') as file_obj:
                requests.post("http://localhost:5001/predict_remote",
                              files={'image': file_obj})
            end_time = time.perf_counter()
            time_array.append(end_time - start_time)
    else:
...
def generate_ground_truth(files):
    ...
    #print('Test in generate ground truth', files)
    with open(ground_truth_path, 'r') as json_file:
        ground_truth = json.load(json_file)

    for file_path in files:
        label = ground_truth.get(file_path)
        #print('this is 2nd test in generate ground truth',file_path)
        labels.append(label)
...
def result_transfer_identity(results_list):  # [[x1, y1], [x2, y2], [...]] \Test
    ...
    for numbers in results_list:
        ...
        #print(len(chosen_number))
        #print(chosen_number)
        # Format the number to keep six digits after the decimal point and concat them
        signatures_list.append(chosen_number)
        #formatted_number = (format(chosen_number, '.5f'))
        #signature_number += formatted_number.replace('.', '')
...
def result_transfer_fairness(results_list, ground_truth):
    ...
    for i in range(len(results_list)):
        if i < mid_results:
            #print('This is a test',binary_result[i], ground_truth[i])
